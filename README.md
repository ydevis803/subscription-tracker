...
Seven anonymous milestones (onboarding started and completed, first renewal check completed, day-two and day-seven return, Premium viewed, Premium conversion) are sent once per profile as `{ event, day }` to `POST /api/analytics`, which only increments a counter per event per day; there is no identifier, account, amount or name in the payload, and the sent flags live in `settings.analyticsSent` so a milestone can never double-count. Failed sends queue in local storage and retry when online. `/__analytics` (owner-only) reads `GET /api/analytics/summary`, which needs `X-Owner-Key` = `OWNER_KEY` (or loopback when unset), and shows counts, conversion rates with their numerator and denominator spelled out, the biggest drop-off, and the single first-week experiment defined in `src/lib/experiment.ts` (one hypothesis, one change, one success metric, one guardrail). The Privacy Policy and store listing state this exactly.
...
A **Google Play preparation** section sits beneath it: final 512 px icon and 1024 × 500 feature graphic with live previews (both exported by `npm run logo:export`), package name, version name and code, Privacy Policy and Terms URLs (same-origin addresses are fetched and must respond 200), and a testing confirmation covering a real device and the closed-test rule for personal accounts. Two conditional items appear only when switched on: the Google Sign-In SHA-256 reminder (register the Play app-signing certificate, not the upload key) and a Google snippet reminder for when Play Console asks for one. Required and conditional items are badged, ready marks need a valid value, and the summary can be copied or downloaded.
