...
// Anonymous analytics: same-origin only, allowlisted events. (Adds one premium_view count dated 2026-01-01 to the dev database.)
const evOk = await anon.call('POST', '/api/analytics', { event: 'premium_view', day: '2026-01-01' })
check('analytics: allowlisted event from the app is accepted', evOk.status === 200, `status ${evOk.status}`)
const evBad = await anon.call('POST', '/api/analytics', { event: 'user_email', day: '2026-01-01', email: 'x@y.z' })
check('analytics: unknown event names are rejected', evBad.status === 400, `status ${evBad.status}`)
const evCsrf = await anon.call('POST', '/api/analytics', { event: 'premium_view', day: '2026-01-01' }, { headers: { Origin: 'https://evil.example' } })
check('analytics: cross-site posts are refused', evCsrf.status === 403, `status ${evCsrf.status}`)
