...
await page.locator('input[autocomplete$="password"]').first().fill('a long enough passphrase')
...
// ---------- Legal, support and deletion (reachable before any account) ----------
await go('/legal/privacy')
await shot('legal-privacy')
await go('/legal/terms')
await shot('legal-terms')
await go('/support')
await shot('support')
await go('/account/delete')
await shot('delete-account')
await page.getByRole('button', { name: /Delete my account|Erase all data on this device/ }).first().click()
await shot('sheet-delete-confirm', { fullPage: false })
await page.keyboard.press('Escape')
