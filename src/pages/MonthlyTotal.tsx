        ...
        {!model || !settings ? (
          ...
          <>
            <div className="rounded-3xl bg-navy-900 p-5 text-white">
              <p className="text-[13px] font-semibold uppercase tracking-wide text-mint-400">Right now</p>
              <p className="tabular mt-1 text-[40px] font-bold leading-none">{formatMoney(model.total, currency)}</p>
              ...
              {model.delta !== null && model.previous && (
                <p className={`mt-3 inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-[13px] font-semibold ${model.delta > 0.005 ? 'bg-coral-500/20 text-coral-300' : model.delta < -0.005 ? 'bg-mint-500/15 text-mint-300' : 'bg-white/10 text-navy-100'}`}>
            ...
            <Card className="p-4">
              <p className="text-[15px] font-bold text-navy-900">How this number is made</p>
              ...
              <ul className="mt-3 grid grid-cols-2 gap-2">
                {RULES.map((r) => (
                  <li key={r.cycle} className="rounded-xl bg-navy-50 px-3 py-2 text-[13px]">
                      ...
                      <li key={sub.id}>
                        <button type="button" onClick={() => navigate(`/subscriptions/${sub.id}`)} className="flex min-h-12 w-full items-center gap-3 py-2 text-left">
                          ...
                          <span className="min-w-0 flex-1">
                            <span className="block text-[14px] font-semibold text-ink">{sub.name}</span>
                          ...
                          <span className="tabular text-[14px] font-bold text-navy-900">{formatMoney(monthly, currency)}</span>
                        </button>
                      </li>
                    ))}
                  </ul>
            ...
            <Card className="p-4">
              <p className="text-[15px] font-bold text-navy-900">Your monthly limit</p>
              <p className="mt-1 text-[14px] leading-relaxed text-muted">
              ...
              <form onSubmit={save} className="mt-3 space-y-3" noValidate>
                <label className="block">
                  <span className="mb-1.5 block text-[13px] font-semibold text-navy-800">Limit per month</span>
                  <span className="relative block">
                    ...
                    <input
                      ...
                      className={`h-13 w-full rounded-2xl border bg-white pl-12 pr-4 text-[16px] outline-none focus:border-navy-600 focus:ring-4 focus:ring-navy-600/10 ${error ? 'border-coral-500' : 'border-line'}`}
                    />
                  </span>
                  <span id="limit-help" className={`mt-1.5 block text-[13px] ${error ? 'font-medium text-coral-700' : belowTotal ? 'font-medium text-coral-700' : 'text-muted'}`} role={error ? 'alert' : undefined}>
                ...
                {suggestion !== null && budget === null && draft === null && (
                  <button type="button" onClick={() => setDraft(String(suggestion))} className="flex h-11 items-center gap-2 rounded-full bg-mint-100 px-4 text-[13px] font-semibold text-mint-700">
              ...
              {limitHistory.length > 0 && (
                <div className="mt-4">
                  <p className="text-[12px] font-semibold uppercase tracking-wide text-faint">Limit history</p>
                  <ul className="mt-1 divide-y divide-line" aria-label="Limit history, newest first">
                    {limitHistory.slice(0, 6).map((h, i) => (
                      <li key={h.changedAt + i} className="flex items-center justify-between py-2 text-[13px]">
            ...
            <Card className="p-4">
              <p className="text-[15px] font-bold text-navy-900">Last six months</p>
              <p className="mt-1 text-[13px] text-muted">Rebuilt from start dates, cancellations and price changes, so it is only as complete as what you have entered.</p>
              <div className="mt-4 flex items-end gap-2">
                {model.history.map((p) => (
                  <div key={p.key} className="flex flex-1 flex-col items-center gap-1" title={`${p.label}: ${formatMoney(p.total, currency)}`}>
                    <span className="tabular text-[10px] font-semibold text-muted">{formatMoney(p.total, currency, { compact: true }).replace(/\.00$/, '')}</span>
                    ...
                    <span className={`text-[11px] font-medium ${p.isCurrent ? 'text-navy-900' : 'text-faint'}`}>{p.label}</span>
                  </div>
                ))}
              </div>
              <ul className="mt-3 divide-y divide-line" aria-label="Monthly totals, newest first">
                {[...model.history].reverse().map((p) => (
                  <li key={p.key} className="flex items-center justify-between py-2 text-[13px]">
